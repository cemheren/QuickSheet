...
- Stars: 1 (? +1 since last run)
- Action: Created quicksheet-gha extension � live GitHub Actions workflow run statuses on desktop wallpaper. Shows ????????? status, workflow name, branch, and age for any public GitHub repo. Optional GITHUB_TOKEN for private repos.
- Bucket: F (new extension)
...
- Stars: 0 (delta -1)
- Action: Fixed issue #154 -- extension prefixes with trailing colon never activated. TrimEnd(':') on registration.
- Bucket: Issue fix (priority)
- Outcome: PR #155 (commit 382f770 on grow/fix-prefix-trailing-colon). Build green.
- Competitor last did: idle (last run 2026-05-19).
- Follow-up: None -- issues-only mode per user instruction.

## Queued

- Issues-only mode: act only on new issues, skip otherwise

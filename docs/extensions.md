...
| `rss` | RSS/Atom Feeds | Live headlines from any RSS or Atom feed on your desktop. `rss: https://hnrss.org/newest?points=50` for a single feed, or `rss: ~/feeds.txt` for multi-feed aggregation. Sorted by date, no API key. | [`quicksheet-rss-ext`](https://github.com/cemheren/quicksheet-rss-ext) |

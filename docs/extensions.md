...
| `moon`    | Lunar Phase       | Live lunar phase widget — fully offline, deterministic, no API key. `moon:` (today's phase emoji, name, illumination %, age), `moon: 2026-12-25` (phase on a date), `moon: next full`, `moon: next new`. Computed from the synodic month; great as an ambient wallpaper cell. | [`quicksheet-moon-ext`](https://github.com/Deskworks/quicksheet-moon-ext) |

...
| `books` | Book Lookup | Book lookup via Open Library. `books: Dune` shows title, author, year, publisher, pages. `books: search Tolkien` returns a table of results. `books: 978-0-13-468599-1` for ISBN lookup. Free API, no key, in-memory cache. | [`quicksheet-books-ext`](https://github.com/cemheren/quicksheet-books-ext) |

...
| `tldr`    | TLDR Pages        | Command cheatsheets from tldr-pages — formatted examples for any CLI command. `tldr: tar`, `tldr: git-rebase`, `tldr: ffmpeg`. No API key, 30-min cache, zero NuGet. | [`quicksheet-tldr-ext`](https://github.com/cemheren/quicksheet-tldr-ext) |

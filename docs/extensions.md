...
| `git`     | Git Status        | Git repository status in cells. `git: status` (branch + changes + ahead/behind), `git: branch`, `git: log [N]` (recent commits), `git: diff` (modified files), `git: stash`, `git: remote`. Zero NuGet, uses git CLI. | [`quicksheet-git`](https://github.com/cemheren/quicksheet-git) |

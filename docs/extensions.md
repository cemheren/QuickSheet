...
| `nuget`   | NuGet Packages    | .NET package lookup — version, total downloads, license, dotnet CLI install command. `nuget: Newtonsoft.Json`. No API key, 30-min cache. | [`quicksheet-nuget`](https://github.com/cemheren/quicksheet-nuget) |
| `mvn`     | Maven Central     | Java/Kotlin package lookup — version, download count, license, description. `mvn: com.google.guava:guava`. No API key, 30-min cache. | [`quicksheet-maven`](https://github.com/cemheren/quicksheet-maven) |
| `payroll` | US Payroll        | Payroll withholding estimator — federal income tax, Social Security, Medicare, net take-home per pay period. 2025 IRS brackets. `payroll: 3846.15, biweekly, single`. | [`quicksheet-payroll-ext`](https://github.com/cemheren/quicksheet-payroll-ext) |
| `tax`     | US Sales Tax      | State sales tax rate lookup — instant rates for all 50 states + DC. `tax: CA` · `tax: NY`. | [`quicksheet-salestax-ext`](https://github.com/cemheren/quicksheet-salestax-ext) |
| `init`    | Initiative Tracker | Combat initiative tracker for TTRPG GMs — sort combatants by roll, cycle turns, track rounds. `init: Goblin 15, Fighter 18, Mage 12`. | [`quicksheet-init-ext`](https://github.com/cemheren/quicksheet-init-ext) |
| `ollama`  | Local AI (Ollama) | Local LLM queries via Ollama — private, fast, no API key. Ask questions, summarize, translate from cells. `ollama: summarize this paragraph`. | [`quicksheet-ollama`](https://github.com/cemheren/quicksheet-ollama) |
| `ai:`     | AI Cost Tracker   | Monitor AI API token usage, costs, and budget across OpenAI, Anthropic, Gemini, DeepSeek. `ai: openai` · `ai: budget`. | [`quicksheet-ai-costs`](https://github.com/cemheren/quicksheet-ai-costs) |
| `words:`  | Text Analysis     | Word count, reading time, readability grade (Flesch-Kincaid), character count for writers. `words: <cell range or text>`. | [`quicksheet-words`](https://github.com/cemheren/quicksheet-words) |
| `hash:`   | Hash Generator    | Generate MD5, SHA1, SHA256, SHA512, CRC32 hashes from cell text or file paths. `hash: hello world` · `hash: sha512 mytext`. | [`quicksheet-hashgen`](https://github.com/cemheren/quicksheet-hashgen) |
| `unit`    | Unit Converter    | Instant conversion — length, mass, temperature, volume, speed, data, area, time. `unit: 5km to miles` · `unit: 100F to C`. | [`quicksheet-unit-ext`](https://github.com/cemheren/quicksheet-unit-ext) |
| `pubmed`  | PubMed Lookup     | Search PubMed articles, fetch metadata by PMID — title, authors, journal, year, DOI. `pubmed: 12345678` · `pubmed: search CRISPR`. | [`quicksheet-pubmed-ext`](https://github.com/cemheren/quicksheet-pubmed-ext) |

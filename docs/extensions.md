...
| `aqi`     | Air Quality       | Live air quality for any city via Open-Meteo (free, no API key). US EPA AQI value + category, PM2.5 and PM10 concentrations. `aqi: London` · `aqi: Delhi` · `aqi: 37.77,-122.42`. Pairs with the weather extension for an ambient air+weather row. Zero NuGet. | [`quicksheet-aqi-ext`](https://github.com/cemheren/quicksheet-aqi-ext) |

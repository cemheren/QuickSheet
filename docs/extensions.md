...
| `mvn`     | Maven Central     | Java/JVM package lookup from Maven Central. Version, version count, publish date, packaging, Maven XML + Gradle snippets. `mvn: org.springframework:spring-core` (detail), `mvn: search logging` (top 8). No API key, 30-min cache, zero NuGet. | [`quicksheet-maven`](https://github.com/cemheren/quicksheet-maven) |
| `nuget`   | NuGet Packages    | .NET package lookup from NuGet Gallery. Version, total downloads, authors, license, description, `dotnet add` + PackageReference snippets. `nuget: Newtonsoft.Json` (detail), `nuget: search json serializer` (top 8). No API key, 30-min cache, zero NuGet deps. | [`quicksheet-nuget`](https://github.com/cemheren/quicksheet-nuget) |

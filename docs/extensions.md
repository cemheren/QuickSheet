...
| `stocks`  | Stock & Crypto Ticker | Live quotes via Yahoo Finance, no API key, 60s cache. Single ticker detail view (price, ▲▼ change, day range, 52W, volume, market cap) or multi-ticker comparison table. Supports stocks, ETFs, indices (^GSPC), crypto (BTC-USD, ETH-USD), forex (EURUSD=X). Usage: `stocks: AAPL` · `stocks: AAPL, MSFT, BTC-USD`. | [`quicksheet-stocks`](https://github.com/Deskworks/quicksheet-stocks) |
| `dict`    | Dictionary        | English dictionary — definitions, phonetics, part-of-speech, usage examples, synonyms and antonyms via dictionaryapi.dev. `dict: serendipity` (full entry), `dict: syn happy` (synonyms only), `dict: ant brave` (antonyms only). No API key, 1h cache. | [`quicksheet-dict`](https://github.com/Deskworks/quicksheet-dict) |

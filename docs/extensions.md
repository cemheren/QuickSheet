...
| `gomod`   | Go module info    | Latest version, publish date, and published-version count for any Go module path (e.g. `gomod: github.com/gin-gonic/gin`). Multi-module comparison table. Via the public Go module proxy, no API key, 30-min cache. | [`quicksheet-gomod`](https://github.com/Deskworks/quicksheet-gomod) |

...
| `unit`    | Unit Converter    | Universal unit converter — length, weight, temperature, data (SI+binary), speed, time, volume, area, pressure. Offline, no API key. `unit: 100 km to mi` · `unit: 32 f c` · `unit: 1024 mb gb` | [`quicksheet-unitconv`](https://github.com/cemheren/quicksheet-unitconv) |

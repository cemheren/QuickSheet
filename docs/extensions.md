...
| `dict`    | Dictionary (full) | English dictionary with synonyms & antonyms. `dict: <word>` for definitions and phonetics, `dict: syn <word>`, `dict: ant <word>`. | [`quicksheet-dict`](https://github.com/Deskworks/quicksheet-dict) |
| `gitlog`  | Git commit log    | Recent git commits in a cell — see repo activity at a glance. `gitlog: <path_or_count>`. | [`quicksheet-gitlog`](https://github.com/Deskworks/quicksheet-gitlog) |
| `stocks`  | Stock/crypto ticker | Live stock and crypto quotes via Yahoo Finance. `stocks: AAPL` or `stocks: AAPL, MSFT, BTC-USD`. No API key. | [`quicksheet-stocks`](https://github.com/Deskworks/quicksheet-stocks) |

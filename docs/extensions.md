...
| `mvn`     | Maven Central     | Java/JVM package lookup from Maven Central. `mvn: org.springframework:spring-core` shows version, Maven/Gradle snippet, packaging, publication date. `mvn: search logging` lists top 8 results. No API key, 30-min cache, zero NuGet. | [`quicksheet-maven`](https://github.com/cemheren/quicksheet-maven) |

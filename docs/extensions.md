...
| `ai`      | AI Cost Tracker   | Monitor AI API costs on your wallpaper — OpenAI usage, model pricing table (15 models), cost calculator, monthly budget tracker, provider comparison. `ai:` (summary), `ai: openai` (today's usage), `ai: calc 100k claude-sonnet-4` (estimate), `ai: budget` (spend vs limit), `ai: compare` (all providers). | [`quicksheet-ai-costs`](https://github.com/cemheren/quicksheet-ai-costs) |

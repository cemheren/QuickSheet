...
public class GridManager
{
    ...
    public void LoadFromCsv(string path) => LoadFromDelimited(path, ',');

    public void LoadFromDelimited(string path, char delimiter)
    {
        if (!File.Exists(path)) return;
        var lines = File.ReadAllLines(path);
        for (int r = 0; r < Math.Min(lines.Length, RowCount); r++)
        {
            var fields = ParseDelimitedLine(lines[r], delimiter);
